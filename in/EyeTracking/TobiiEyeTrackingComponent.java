...
/**
 * Provides eye tracking ability using tobii eye tracker for agentslang.
 * OS Compatibility: Windows
 * @author sahba ZOJAJI, sahba.zojaji@insa-rouen.fr
 * @version 1, 02/11/2017 - Initial commit, sahba ZOJAJI
 */
@ConfigureParams(outputChannels = {"tobiiEyeTracking.data"},
outputDataTypes = {StringData.class})
public class TobiiEyeTrackingComponent extends MixedComponent implements TobiiSocket.DataListener {
	...
	private TobiiSocket socket;
	
	protected TobiiEyeTrackingComponent(String outboundPort, ComponentConfig config) {
	...
	protected void setupComponent(ComponentConfig config) {
        ...
        System.out.println(String.format("Using Tobii Eye Tracker Connections Properties hostname='%s' inPort=%d outPort=%d", hostname, intPort, outPort));

        try {
        	socket = new TobiiUDPSocket(hostname, intPort, outPort, this);
        } catch (IOException e) {
            Logger.log(this, Logger.CRITICAL, "Tobii Eye Tracker Invalid properties for socket", e);
            System.out.println("Tobii Eye Tracker Invalid properties for socket" + e.toString());
	...
	/**
	 * handles received data from tobii eye tracker
	 * @param message received message from eye tracking application
	 */
	@Override
	public void dataReceived(String message) {
        Logger.log(this, Logger.INFORM, "From Tobii Eye Tracker : " + message);
